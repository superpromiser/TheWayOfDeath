...
import {getTemplateList,deleteTemplate} from '~/api/schoolStory'
export default {
    data:()=>({
        lang,
        isSubmit:false,
    ...
    methods:{
        submit(){
            console.log('submit')
            this.$router.push({name:'schoolStory.newTemplate'})
        },
        selTemp(tempData){
            this.$router.push({name:'posts.schoolStory',query:{tempData:JSON.stringify(tempData.content)}})
        },
