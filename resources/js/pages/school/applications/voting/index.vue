<template>
    <v-container class="pa-0" v-if="$isMobile()">
        <v-container class="pt-0 px-0 h-100 bg-white mb-16 pb-10-px pt-12">
            ...
            <v-container v-if="contentList.length && content.votings" class="pa-0" v-for="content in contentList" :key="content.id" >
            ...
            >   
                ...
                <div slot="no-more" class="pa-3 ma-3 text-center ma-0">
                    <v-chip
                    class="ma-2"
                    color="#7879ff"
                    outlined
...
export default {
    ...
    methods:{
        async infiniteHandler($state){
            ...
            let vm = this;
            await getVoting({page:this.pageOfContent,schoolId:this.currentPath.params.schoolId})
            .then(res=>{
