    ...
    <v-container class="pa-0" v-if="$isMobile()">
        <v-container class="pt-0 px-0 h-100 bg-white mb-16 pb-10-px pt-12">
            <v-row class="ma-0 bg-white justify-center position-sticky-top-0" >
                ...
                <v-btn v-if="user.roleId != 6" @click="post" text color="#7879ff" class="position-absolute put-align-center" style="right: 0px; top:50%">
                    发布
                </v-btn>
            </v-row>
            <div class="cus-divider-light-gray-height"></div>
            <v-container v-if="contentList.length && content.shares" class="pa-0" v-for="content in contentList" :key="content.id" >
...
export default {
    ...
    methods:{
        async infiniteHandler($state){
            ...
            let vm = this;
            await getShare({page:this.pageOfContent,schoolId:this.currentPath.params.schoolId})
            .then(res=>{
