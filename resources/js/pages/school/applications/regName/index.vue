    ...
    <v-container class="pa-0" v-if="$isMobile()">
        <v-container class="pt-0 px-0 h-100 bg-white mb-16 pb-10-px">
            <v-row class="ma-0 bg-white justify-center position-sticky-top-0" >
                ...
                <v-btn v-if="user.roleId == 1 || user.roleId == 2 || user.roelId == 3 || user.roleId == 7"
...
import {getRegname} from '~/api/regname';
...
export default {
    ...
    methods:{
        async infiniteHandler($state){
            ...
            let vm = this;
            await getRegname({page:this.pageOfContent,schoolId:this.currentPath.params.schoolId})
            .then(res=>{
        ...
        post(){
            this.$router.push({name:"posts.regname"})
        }
    }
}
</script>
