...
export default {
  ...
  async created(){
    this.isLoading = true
    await getUserByRole({schoolId:this.currentPath.params.schoolId}).then(res=>{
    ...
    action(type){
      switch(type){
        ...
        case 'application':
          ...
          break;
        case 'import':
          this.$router.push({name:'schoolSpace.importMember', params:{schoolId:this.currentPath.params.schoolId}});
          break;
        case 'group':
