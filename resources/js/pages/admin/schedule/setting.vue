...
export default {
  ...
  async created() {
    ...
    if (this.mySchoolData.grades.length > 0) {
      ...
      await getSchedule({ gradeId: this.currentSelectedGrade })
        .then(res => {
  ...
  methods: {
    ...
    async deleteItemConfirm() {
      ...
      await deleteSchedule({ id: this.editedIndex })
        .then(res => {
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        await updateSchedule(this.editedItem)
          .then(res => {
            Object.assign(
              this.scheduleSettingData[this.editedIndex],
              this.editedItem
            );
          })
          .catch(err => {
      ...
      //save scheduleSettingData
        ...
        await createSchedule(this.editedItem)
          .then(res => {
