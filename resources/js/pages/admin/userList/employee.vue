                    ...
                    <v-container>
                      ...
                      <v-row class=" align-center">
                        ...
                        >
                          <v-avatar
                            v-if="
                              editedItem.name !== '' && editedItem.avatar == '/'
                            "
                            color="#7879ff"
                            size="127"
                            tile
                          ...
                          <img
                            ...
                            "
                            :src="
                              `${baseUrl}/asset/img/icon/anonymous_avatar.png`
                            "
                            alt="UserAvatar"
                            style="width:127px; height: 127px;"
                          />
                          <img
                            v-else-if="editedItem.avatar !== '/'"
  ...
  methods: {
    ...
    async save() {
      ...
      if (this.editedItem.phoneNumber.trim() == "") {
        return this.$snackbar.showMessage({
          content: this.lang.requireCorrectPhoneNumber,
          color: "error"
        });
      }
      if (/^\d*$/.test(this.editedItem.phoneNumber) == false) {
        return this.$snackbar.showMessage({
          content: this.requireCorrectPhoneNumber,
          color: "error"
        });
      }
      ...
      if (this.editedItem.gender == null) {
        return this.$snackbar.showMessage({
          content: this.lang.requireGender,
          color: "error"
        });
      }
      //nation
      if (this.editedItem.nation == "") {
        return this.$snackbar.showMessage({
          content: this.lang.requireNation,
          color: "error"
        });
      }
      //cardnumber
      if (this.editedItem.cardNum.toString().trim() == "") {
        return this.$snackbar.showMessage({
          content: this.lang.requireCardNumber,
          color: "error"
        });
      }
      if (/^\d*$/.test(this.editedItem.cardNum) == false) {
        return this.$snackbar.showMessage({
          content: this.lang.requireCorrectCardNumber,
          color: "error"
        });
      }
      ...
      if (this.editedIndex > -1) {
        ...
        await updateEmployee(this.editedItem)
          .then(res => {
            ...
            Object.assign(
              this.schoolManagerData[this.editedIndex],
              this.editedItem
            );
            this.close();
          })
