  ...
  <v-container>
    <v-row>
      ...
      <v-col cols="6" class="d-flex justify-end">
        <v-btn color="#f89651" dark class="mx-2" @click="cancelUpload">
          <!-- <v-icon left>
                        mdi-file-upload
                    </v-icon> -->
          取消
        </v-btn>
        <v-btn color="#7879ff" dark :loading="isUploading" @click="uploadExcel">
          <!-- <v-icon left>
                        mdi-file-upload
                    </v-icon> -->
          保存
...
export default {
  ...
  methods: {
    ...
    async uploadExcel() {
      ...
      await uploadExcelApi({ serverData: serverData })
        .then(res => {
