...
export default {
  ...
  async created() {
    ...
    getLessonItem({ schoolId: this.user.schoolId })
      .then(res => {
        res.data.map(data => {
          let lessonItem = {};
  ...
  methods: {
    ...
    async deleteItemConfirm() {
      ...
      console.log(this.editedItem);
      await deleteLessonAttendanceData({ id: this.editedItem.id })
        .then(res => {
    ...
    async save() {
      ...
      this.editedItem.resultNormal = this.getNumberOfAttendanceResult(
        "normal",
        this.editedItem
      );
      this.editedItem.resultLate = this.getNumberOfAttendanceResult(
        "late",
        this.editedItem
      );
      this.editedItem.resultMiss = this.getNumberOfAttendanceResult(
        "miss",
        this.editedItem
      );
      this.editedItem.resultRest = this.getNumberOfAttendanceResult(
        "rest",
        this.editedItem
      );
      console.log(this.editedItem);
      ...
      if (this.editedItem.attendanceTime == "") {
        return this.$snackbar.showMessage({
          content: this.lang.requiredlessonName,
          color: "error"
        });
      }
      this.isCreatingSchool = true;
      if (this.editedIndex > -1) {
        await updateLessonAttendanceData(this.editedItem)
          .then(res => {
            this.isCreatingSchool = false;
            Object.assign(
              this.attendanceData[this.editedIndex],
              this.editedItem
            );
          })
          .catch(err => {
      ...
      //save attendanceData
        ...
        await createLessonAttendanceData(this.editedItem)
          .then(res => {
            console.log(res.data);
            this.isCreatingSchool = false;
            if (res.data.status == 207) {
              return this.$snackbar.showMessage({
                content: this.lang.alreadyExist,
                color: "error"
              });
            } else {
      ...
      switch (type) {
        case "normal":
          item.resultArr.map(x => {
            if (
              x.attendanceResult == "正常出勤" ||
              x.attendanceResult == "出勤(带病)"
            ) {
              counter++;
            }
          });
