<template>
  <v-container class="ma-0 pa-0 h-100" v-if="$isMobile()">
    <v-container class="pt-0 px-0 h-100 bg-white mb-16 pb-10-px pt-12">
      <v-row class="ma-0 bg-white justify-center position-sticky-top-0">
        ...
        <v-btn
          link
          :to="{ name: 'classSpace.newReturnTeam' }"
          text
          color="#7879ff"
          class="position-absolute put-align-center"
      ...
      <v-row v-else-if="noData" class="pa-5 d-flex justify-center align-center">
        <v-chip color="#7879ff" outlined pill>
          暂无
          <v-icon right>
            mdi-cancel
          </v-icon>
      ...
      <v-container v-else class="pa-0">
        <v-row
          v-if="returnTeam.name !== '留堂成员'"
          class="ma-0 hover-cursor-point"
          v-ripple
          v-for="(returnTeam, i) in todayReturnTeamArr"
          :key="i"
          @click="navToDetail(returnTeam)"
        >
          <v-col cols="12" class="d-flex justify-space-between align-center">
            <div class="d-flex align-center">
              <v-avatar
                size="50"
                :color="
                  returnTeam.name == '' && returnTeam.avatar == null
                    ? '#999999'
                    : '#7879ff'
                "
              >
                <v-img
                  v-if="returnTeam.avatar !== null"
                  :src="`${baseUrl}${returnTeam.avatar}`"
                >
                </v-img>
                <span
                  v-else-if="returnTeam.name !== ''"
                  class="white--text headline"
                  >{{ returnTeam.name[0] }}</span
                >
                <v-icon v-else dark>
                  mdi-account
                </v-icon>
            ...
            <div class="d-flex align-center">
              <p class="mb-0">
                {{ returnTeam.leader_id.name }} 和另外
                {{ returnTeam.member.length - 1 }}个人
              </p>
          ...
          <div
            v-if="i < todayReturnTeamArr.length - 1"
            class="cus-divider-light-gray-height"
          ></div>
        </v-row>
        <v-divider
          v-if="remainTeam !== null"
          light
          class="thick-border"
        ></v-divider>
        <v-row
          ...
          <v-col cols="12" class="d-flex justify-space-between align-center ">
            <div class="d-flex align-center">
              <v-avatar
                size="50"
                :color="
                  remainTeam.name == '' && remainTeam.avatar == null
                    ? '#999999'
                    : '#7879ff'
                "
              >
      ...
      <v-btn
        link
        :to="{ name: 'classSpace.remainReturnTeam' }"
        rounded
        color="#E0E0E0"
        small
        elevation="0"
      ...
      <v-btn
        :dark="!isExistTodayRemainTeam"
        :disabled="isExistTodayRemainTeam"
        link
        :to="{ name: 'classSpace.newRemainTeam' }"
        rounded
        color="#E0E0E0"
        small
        elevation="0"
  ...
  <v-container class="pa-0" v-else>
    ...
    <v-container v-if="isLoading" class="d-flex align-center justify-center">
      <v-progress-circular indeterminate color="#7879ff"></v-progress-circular>
    </v-container>
    ...
    <v-container v-else class="pa-0">
      ...
      >
        <v-col
          cols="12"
          class="d-flex justify-space-between align-center pl-10 pr-6"
        >
          <div class="d-flex align-center">
            <v-avatar
              size="50"
      ...
      await getReturnTeam()
        .then(res => {
          res.data.returnTeamArr.map(returnTeam => {
            ...
            if (
              returnTeam.name == "留堂成员" &&
              this.isToday(returnTeam.updated_at) == true
            ) {
              todayTeamArr.push(returnTeam);
