  ...
  <v-container class="ma-0 pa-0 h-100" v-if="$isMobile()">
    <v-container class="pt-0 px-0 h-100 bg-white mb-16 pb-10-px pt-12">
      <v-row class="ma-0 bg-white justify-center position-sticky-top-0">
        ...
        <p
          v-if="isRemainTeam"
          class="mb-0 font-size-0-95 font-weight-bold pa-3"
        >
          留堂成员的详细信息
        </p>
        <p v-else class="mb-0 font-size-0-95 font-weight-bold pa-3">
          留堂成员的详细信息
        </p>
        <v-btn
          v-if="
            user.roleId == 1 ||
              ...
              user.roleId == 7
          "
          :dark="!isDetailView || !isRemainTeam"
          :disabled="isDetailView || isRemainTeam"
          @click="submit"
          :loading="isUpdating"
          text
          color="#7879ff"
      ...
      >
          ...
          <v-col cols="12" class="d-flex justify-space-between align-center">
            ...
            <div class="d-flex align-center">
              <v-avatar
                size="50"
                :color="
                  teamData.name == '' && teamData.avatar == null
                    ? '#999999'
                    : '#7879ff'
                "
              >
                <v-img
                  v-if="teamData.avatar !== null"
                  :src="`${baseUrl}${teamData.avatar}`"
                >
                </v-img>
                <span
                  v-else-if="teamData.name !== ''"
                  class="white--text headline"
                  >{{ teamData.name[0] }}</span
                >
                <v-icon v-else dark>
                  mdi-account
                </v-icon>
        ...
        <v-row class="ma-0 hover-cursor-point" v-ripple @click="navToAddMember">
          <v-col cols="12" class="d-flex justify-space-between align-center">
            ...
            <div class="d-flex align-center">
              <p
                v-if="teamData.member.length !== 0"
                class="mb-0 secondary-text"
              >
                {{ teamData.member.length }} 个已选择
              </p>
              <v-icon class="ml-2" color="#999999" size="25">
                mdi-chevron-right
              </v-icon>
      ...
      <v-container class="pa-0">
        <v-row class="ma-0 py-2">
          ...
          <v-col
            ...
            v-else
            v-for="(user, i) in teamData.member"
            :key="i"
            cols="12"
            sm="6"
            md="4"
          ...
          >
            ...
            <div
              v-if="i < teamData.member.length - 1"
    ...
    >
      ...
      >
        ...
        >
          ...
          <div class="d-flex align-center">
            <p v-if="teamData.member.length !== 0" class="mb-0 secondary-text">
              {{ teamData.member.length }} 个已选择
            </p>
            <v-icon class="ml-4" color="#999999" size="40">
              mdi-chevron-right
      ...
      <v-row class="ma-0 py-2 pl-8 pr-8">
        ...
        >
          <v-card class="text-center pt-4 position-relative">
            ...
            <v-chip
              v-if="
                teamData.leader_id !== null && user.id == teamData.leader_id.id
              "
              dark
  ...
  methods: {
    ...
    navToAddMember() {
      ...
      this.$store.dispatch(
        ...
        this.teamData.avatar
      );
      this.$store.dispatch(
        "returnteam/storeReturnTeamLeader",
        this.teamData.leader
      );
      ...
      await updateReturnTeam(payload)
        .then(res => {
          if (res.data.msg == 1) {
            ...
            let clonedTodayReturnArr = JSON.parse(
              JSON.stringify(this.todayReturnTeamArr)
            );
