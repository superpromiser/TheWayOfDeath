    ...
    <v-container class="pt-0 px-0 h-100 bg-white mb-16 pb-10-px pt-12">
      ...
      <v-container class="pa-0">
          ...
          <v-col cols="12" class="d-flex justify-space-between align-center ">
            ...
            <div class="d-flex align-center">
              <v-avatar
                size="50"
                :color="
                  returnTeamData.name == '' && returnTeamData.avatar == null
                    ? '#999999'
                    : '#7879ff'
                "
              >
                <v-img
                  v-if="returnTeamData.avatar !== null"
                  :src="`${baseUrl}${returnTeamData.avatar}`"
                >
                </v-img>
                <span
                  v-else-if="returnTeamData.name !== ''"
                  class="white--text headline"
                  >{{ returnTeamData.name[0] }}</span
                >
                <v-icon v-else dark>
                  mdi-account
                </v-icon>
        ...
        <v-row
          class="ma-0  hover-cursor-point"
          v-ripple
          @click="$refs.returnTeamName.focus()"
        >
          <v-col cols="12" class="d-flex justify-space-between align-center ">
        ...
        <v-row
          class="ma-0  hover-cursor-point"
          v-ripple
          @click="navToAddTeacher"
        >
          <v-col cols="12" class="d-flex justify-space-between align-center ">
            <p class="mb-0 font-size-0-8">领队教师</p>
            <div class="d-flex align-center">
              <p
                v-if="returnTeamData.teacher !== null"
                class="mb-0 secondary-text"
              >
                {{ returnTeamData.teacher.name }}
              </p>
              <v-icon class="ml-2" color="#999999" size="25">
                mdi-chevron-right
              </v-icon>
        ...
        <v-row
          class="ma-0  hover-cursor-point"
          v-ripple
          @click="navToAddMember"
        >
          <v-col cols="12" class="d-flex justify-space-between align-center">
            <p class="mb-0 font-size-0-8">归程队成员</p>
            <div class="d-flex align-center">
              <p
                v-if="returnTeamData.member.length !== 0"
                class="mb-0 secondary-text"
              >
                {{ returnTeamData.member.length }} 个已选择
              </p>
              <v-icon class="ml-2" color="#999999" size="25">
                mdi-chevron-right
              </v-icon>
        ...
        >
          <v-col cols="12" class="d-flex justify-space-between align-center">
            ...
            <div class="d-flex align-center">
              <p
                v-if="returnTeamData.leader !== null"
                class="mb-0 secondary-text"
              >
                {{ returnTeamData.leader.name }}
              </p>
              <v-icon class="ml-2" color="#999999" size="25">
                mdi-chevron-right
              </v-icon>
        ...
        <v-row class="ma-0 ">
          ...
          <v-col
            ...
            v-else
            v-for="(user, i) in returnTeamData.member"
            :key="i"
            cols="12"
            sm="6"
            md="4"
          ...
          >
            <div class="w-100 d-flex align-center">
              ...
              <v-chip
                v-if="
                  returnTeamData.leader !== null &&
                    user.id == returnTeamData.leader.id
                "
                dark
                color="#7879ff"
              >
            ...
            <div
              v-if="i < returnTeamData.member.length - 1"
              class=" cus-divider-light-gray-height"
            ></div>
          </v-col>
        </v-row>
        <v-dialog
        ...
        >
          <v-card>
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn text color="#7879ff" @click="onSubmitSelectedLeader">
                {{ lang.ok }}
              </v-btn>
            </v-card-actions>
          </v-card>
  ...
  <v-container class="pa-0" v-else>
    ...
    <v-container class="pa-0">
      ...
      >
        <v-col
          cols="12"
          class="d-flex justify-space-between align-center pl-10 pr-8"
        >
          <p class="mb-0">头像</p>
          <div class="d-flex align-center">
            <v-avatar
      ...
      <v-divider light></v-divider>
      <v-row
        class="ma-0 py-2 hover-cursor-point"
        v-ripple
        @click="$refs.returnTeamName.focus()"
      >
        <v-col
          cols="12"
          class="d-flex justify-space-between align-center pl-10 pr-12"
        >
...
export default {
  ...
  methods: {
      ...
      await createReturnTeam(payload)
        .then(res => {
          ...
          this.$set(
            this.returnTeamData,
            "leader_id",
            this.returnTeamData.leader
          );
          this.$set(
            this.returnTeamData,
            "teacher_id",
            this.returnTeamData.teacher
          );
          ...
          if (this.todayReturnTeamArr.length == 0) {
            ...
            this.$store.dispatch(
              "returnteam/storeTodayReturnTeamArr",
              this.todayReturnTeamArr
            );
          }
          this.$store.dispatch("member/storeSelectedGroup", null);
    ...
    navToAddMember() {
      this.$store.dispatch(
        "returnteam/storeReturnTeamName",
        this.returnTeamData.name
      );
      this.$store.dispatch(
        "returnteam/storeReturnTeamAvatar",
        this.returnTeamData.avatar
      );
      this.$store.dispatch(
        "returnteam/storeReturnTeamLeader",
        this.returnTeamData.leader
      );
