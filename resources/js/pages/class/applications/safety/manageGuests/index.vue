    ...
    <v-container class="pt-0 px-0 h-100 bg-white mb-16 pb-10-px pt-12">
      ...
      <v-row class="ma-0">
        ...
        <v-col
          ...
          md="6"
          v-else
          v-for="(guest, i) in guestRequestData"
          :key="i"
        >
          <v-card>
            ...
            <v-card-text>
              <v-row
                class="ma-0 d-flex align-center justify-space-between mb-3"
              >
                <h3 class="">姓名: {{ guest.name }}</h3>
                <v-chip v-if="guest.status == 'allow'" color="#7879ff" outlined
                  >致谢</v-chip
                >
                <v-chip v-if="guest.status == 'deny'" color="#F19861" outlined
                  >拒绝</v-chip
                >
              </v-row>
            ...
            <v-card-actions>
              ...
              <v-btn
                text
                color="#7879ff"
                @click="allowGuest(guest)"
                :disabled="guest.status == 'allow'"
                :loading="guest.isAllow"
              >
                同意
              </v-btn>
              <v-btn
                text
                color="#F19861"
                @click="denyGuest(guest)"
                :disabled="guest.status == 'deny'"
                :loading="guest.isDeny"
              >
                拒绝
              </v-btn>
            </v-card-actions>
        ...
        <v-hover v-slot="{ hover }">
          <v-card class="position-relative">
            ...
            <v-expand-transition>
              <div
                v-if="hover"
                class="transition-fast-in-fast-out position-absolute d-flex align-center justify-center guest-control-hover"
              >
...
export default {
  ...
  methods: {
    ...
    listen() {
      Echo.private("newguest." + this.user.id).listen("NewGuest", alarm => {
