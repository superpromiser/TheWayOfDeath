    ...
    <v-container class="pa-0" v-if="$isMobile()">
        <v-container class="pt-0 px-0 h-100 bg-white mb-16 pb-10-px">
            ...
            <v-container v-if="contentList.length" class="pa-0" v-for="content in contentList" :key="content.id" >
                <v-row class="px-3 mt-1" v-if="content.repairdata">
...
export default {
    ...
    methods:{
        async infiniteHandler($state){
            ...
            let vm = this;
            await getRepairData({page:this.pageOfContent,schoolId:this.currentPath.params.schoolId,status:this.viewType,deadline:this.date,lessonId:this.currentPath.params.lessonId})
            .then(res=>{
