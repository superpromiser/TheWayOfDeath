<template>
  <v-container v-if="$isMobile()" class="pa-0">
    <v-container
      v-if="isPosting == true"
      class="pa-0 h-100 bg-white mb-16 pb-3 pt-12"
    >
      <v-row class="ma-0 bg-white justify-center position-sticky-top-0">
        <v-icon
          @click="navToBackCustom"
          size="35"
      ...
      <QuestionItem
        ...
        :isAnnouncement="true"
        :isShareView="true"
        :item="announcementData.content[0]"
        ref="child"
      ...
      <v-btn
        ...
        style="bottom: 54px; left: 12px;"
      >
        <v-icon left>mdi-buffer</v-icon>模板</v-btn
      >
      <v-menu
        top
        offset-y
        ...
        :content-class="
          publishSpecUserList !== null && publishSpecUserList.length > 0
            ? 'box-shadow-none publish-type-menu-with-btn'
            : 'box-shadow-none publish-type-menu'
        "
        tile
        min-width="90"
      >
        <template v-slot:activator="{ on, attrs }">
          ...
          >
            ...
            {{
              announcementData.publishType == "pub"
                ? "公开"
                : announcementData.publishType == "pvt"
                ? "私密"
                : publishSpecUserList == null
                ? "部分看见"
                : `部分看见(${publishSpecUserList.length})`
            }}
          </v-btn>
        </template>
        <div class="pa-3 text-right">
          ...
          >
            <v-radio
              name="announcementData.publishType"
              color="#7879ff"
              label="公开"
              value="pub"
            ></v-radio>
  ...
  methods: {
    ...
    async publishcampusData() {
      ...
      if (this.announcementData.title.trim() == "") {
        return this.$snackbar.showMessage({
          content: this.lang.announcement + this.lang.requireTitle,
          color: "error"
        });
      }
      if (this.announcementData.signName.trim() == "") {
        return this.$snackbar.showMessage({
          content: this.lang.announcement + this.lang.requireSignName,
          color: "error"
        });
      }
      if (this.announcementData.content.length == 0) {
        return this.$snackbar.showMessage({
          content: this.lang.announcement + this.lang.requireContent,
          color: "error"
        });
      }
      ...
      await createAnouncement(this.announcementData)
        .then(res => {
    ...
    chooseSignName() {
      if (this.indexOfSignName == 0) {
        ...
        this.announcementData.signName = this.signNameItems[
          this.indexOfSignName - 2
        ].text;
      }
      this.chooseSignNameDialog = false;
    },
