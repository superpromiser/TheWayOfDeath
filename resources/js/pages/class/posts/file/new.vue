  ...
  <v-container class="pa-0 ">
    <v-row v-if="$isMobile()">
      <v-col cols="12" class="mo-glow d-flex align-center justify-center">
        <v-avatar class="mo-glow-small-shadow">
          <v-img
            :src="`${baseUrl}/asset/img/appIcon/工具/相册.png`"
            alt="postItem"
            width="48"
            height="48"
          ></v-img>
    ...
    <v-row v-else class="d-flex justify-center pt-10  ma-0">
      <v-col cols="12 d-flex justify-center position-relative">
        <h2
          class="red--text text--lighten-1 position-absolute"
          style="top:0; font-size: 3rem;"
        >
          文件
        </h2>
        <img
          :src="`${baseUrl}/asset/img/icon/header-red-ribbon.png`"
          alt="ribbon"
          class="class-ribbon-img"
        />
        <h2 class="position-absolute white--text" style="bottom:55px">
          {{ className }}
        </h2>
      </v-col>
    </v-row>
    ...
    <v-row class="ma-0">
      ...
      <v-col
        v-else
        v-for="(video, index) in videoDataList"
        :key="index"
        cols="12"
        sm="6"
        md="4"
      ...
      >
        ...
        >
          <img
            :src="`${baseUrl}/asset/img/upload_video_img.png`"
            alt="upload-video-icon"
            class="uploaded-video-icon ma-2"
          />
          <div class="font-size-0-75">
            <div>
              <span
                ><strong>{{ video.fileOriName }}</strong></span
              >
            </div>
          ...
          <div class="ml-auto mr-2">
            <v-btn
              icon
              color="blue darken-1"
              @click="openVideoViewDialog(index, video)"
            >
    ...
    >
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn text color="#7879ff" @click="videoViewDialog = false">
            {{ lang.ok }}
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-row class="ma-0 mb-16">
      ...
      <v-col
        v-else
        v-for="(other, index) in fileDataList"
        :key="index"
        cols="12"
        sm="6"
        md="4"
      ...
      >
        ...
        >
          <img
            :src="`${baseUrl}/asset/img/upload_file_img.png`"
            alt="upload-video-icon"
            class="uploaded-video-icon ma-2"
          />
          <div class="font-size-0-75">
            <div>
              <span
                ><strong>{{ other.fileOriName }}</strong></span
              >
            </div>
      ...
      .then(res => {
        ...
        if (this.videoDataList.length == 0) {
          ...
          for (let i = 0; i < this.videoDataList.length; i++) {
            ...
            clonedOption.sources[0].src =
              this.baseUrl + "/uploads/video/" + this.videoDataList[i].fileName;
