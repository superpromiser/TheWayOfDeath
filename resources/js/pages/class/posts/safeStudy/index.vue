...
export default {
  ...
  methods: {
    templateList() {
      ...
      this.$router.push({ name: "CsafeStudy.templateList" });
    },
    async saveDraft() {
    ...
    async submit() {
      ...
      if (this.shareData.content[0].text.trim() == "") {
        return this.$snackbar.showMessage({
          content: this.lang.safeStudy + this.lang.requireContent,
          color: "error"
        });
      }
      ...
      this.isSubmit = true;
      await createSafeStudy(this.shareData)
        .then(res => {
          console.log(res);
          this.isSubmit = false;
          this.clearStore();
    ...
    selectPublishType(val) {
      ...
      if (val == "spec") {
        if (this.shareData.content[0].text.trim() == "") {
          ...
          return this.$snackbar.showMessage({
            content: this.lang.interClassStory + this.lang.requireContent,
            color: "error"
          });
        }
