<template>
  ...
  >
    ...
    <div
      v-if="isNew == false"
      v-for="(item, index) in multiData.multiContentDataArr"
      :key="index"
    >
      <QuestionItem
        class="mt-2"
    ...
    <v-row class="ma-0 position-absolute-bottom-0 w-100 bg-white pa-3 ">
      <v-col cols="12" class="d-flex justify-space-between align-center pa-0">
        <v-btn color="#7879ff" block dark large @click="addMultiContent">
          确认
        </v-btn>
      </v-col>
    </v-row>
  ...
  methods: {
    ...
    addMultiContent() {
      ...
      if (this.isNew == false) {
        ...
        this.multiData.multiContentDataArr = this.multiData.multiContentDataArr.splice(
          0,
          initialLength
        );
      } else {
