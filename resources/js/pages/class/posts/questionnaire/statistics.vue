  ...
  <v-container>
    ...
    <v-container>
      <v-row class="my-10 d-flex align-center">
        <v-btn large rounded dark color="#7879ff" @click="addStatContent">{{
          lang.submit
        }}</v-btn>
      </v-row>
    </v-container>
...
export default {
  ...
  methods: {
    addStatContent() {
      ...
      if (
        this.statData.statDataArr[0].contentData.length == 0 ||
        this.statData.statDataArr[0].sValue == "" ||
        this.statData.statDataArr[0].eValue == "" ||
        this.statData.statDataArr[0].unit == ""
      ) {
