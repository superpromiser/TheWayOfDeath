<template>
  ...
  >
    ...
    <div
      v-if="isNew == false"
      v-for="(item, index) in singleData.singleContentDataArr"
      :key="index"
    >
      <QuestionItem
        class="mt-2"
    ...
    <v-row class="ma-0 position-absolute-bottom-0 w-100 bg-white pa-3 ">
      <v-col cols="12" class="d-flex justify-space-between align-center pa-0">
        <v-btn color="#7879ff" block dark large @click="addSingleContent">
          确认
        </v-btn>
      </v-col>
    </v-row>
  ...
  methods: {
    ...
    addSingleContent() {
      if (this.isNew == false) {
        ...
        this.singleData.singleContentDataArr = this.singleData.singleContentDataArr.splice(
          0,
          initialLength
        );
      } else {
