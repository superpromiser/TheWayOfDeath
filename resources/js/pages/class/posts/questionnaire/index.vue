<template>
  <v-container class="ma-0 pa-0 h-100" v-if="$isMobile()">
    <v-container
      v-if="postNew == true"
      class="pt-0 px-0 h-100 bg-gray-light-dark mb-16 pb-10-px pt-12"
    >
      ...
      <v-row class="ma-0 mo-glow bg-white">
        ...
        <v-col
          cols="12"
          v-for="(data, index) in newQuestionnaireData.content"
          :key="index"
        >
          <!--  single Datas  -->
          <v-row v-if="data.type == 'single'">
            <v-col cols="12">
              <p class="mb-0 d-flex align-center">
                ...
                <v-btn
                  ...
                  color="green"
                  class=""
                  @click="pushDown(index)"
                  :disabled="index == newQuestionnaireData.content.length - 1"
                >
              ...
              <p class="text-wrap pl-3 mb-0 pa-0 ">
                {{ data.singleContentDataArr[0].text }}
              </p>
            </v-col>
            ...
            <v-col
              class="pl-6 pt-0"
              cols="12"
              v-for="(singleData, singleDataIndex) in data.singleContentDataArr"
              :key="singleDataIndex"
              v-if="singleDataIndex !== 0"
            >
              ...
              <AttachItemViewer
                :items="singleData"
                v-if="checkIfAttachExist(singleData)"
              />
            </v-col>
          </v-row>
          <!--  multi Datas  -->
          <v-row v-if="data.type == 'multi'">
            <v-col cols="12">
              ...
              <p class="text-wrap pl-3 mb-0  ">
                {{ data.multiContentDataArr[0].text }}
              </p>
            </v-col>
            ...
            <v-col
              class="pl-6 pt-0"
              cols="12"
              v-for="(multiData, singleDataIndex) in data.multiContentDataArr"
              :key="singleDataIndex"
              v-if="singleDataIndex !== 0"
            >
              <div class="d-flex align-center cursor-pointer">
              ...
              <AttachItemViewer
                :items="multiData"
                v-if="checkIfAttachExist(multiData)"
              />
            </v-col>
          </v-row>
          <!--  qa Datas  -->
          ...
          <v-row v-if="data.type == 'stat'">
            <v-col cols="12">
              ...
              <p class="text-wrap pl-3 mb-0">
                {{ data.statDataArr[0].contentData[0].text }}
              </p>
            </v-col>
            <v-col
              v-if="checkIfAttachExist(data.statDataArr[0].contentData[0])"
            >
          ...
          <v-row v-if="data.type == 'score'">
            <v-col cols="12">
              ...
              <p class="text-wrap pl-3 mb-0 ">
                {{ data.scoringDataArr[0].contentData[0].text }}
              </p>
            </v-col>
            <v-col
              v-if="checkIfAttachExist(data.scoringDataArr[0].contentData[0])"
            >
              <AttachItemViewer
                :items="data.scoringDataArr[0].contentData[0]"
              />
            </v-col>
          </v-row>
        </v-col>
      ...
      <v-container class="px-10 z-index-2 banner-custom">
        <v-row>
          ...
          <v-col cols="12" md="4" class="d-flex align-center justify-end">
            <v-btn text color="#999999" @click="selContent('template')">
          ...
          >
            ...
            <v-row v-if="data.type == 'single'">
              ...
              <v-col
                ...
                cols="12"
                v-for="(singleData,
                singleDataIndex) in data.singleContentDataArr"
                :key="singleDataIndex"
                v-if="singleDataIndex !== 0"
              >
                <div>
...
export default {
  ...
  created() {
    ...
    getLessonUserList({ lessonId: this.currentPath.params.lessonId })
      .then(res => {
  ...
  methods: {
    ...
    async submit() {
      ...
      if (this.newQuestionnaireData.title.trim() == "") {
        return this.$snackbar.showMessage({
          content: this.lang.questionnaire + this.lang.requireTitle,
          color: "error"
        });
      }
      if (this.newQuestionnaireData.description.trim() == "") {
        return this.$snackbar.showMessage({
          content: this.lang.questionnaire + this.lang.requireDescription,
          color: "error"
        });
      }
      ...
      if (this.newQuestionnaireData.content.length == 0) {
        return this.$snackbar.showMessage({
          content: this.lang.questionnaire + this.lang.requireContent,
          color: "error"
        });
      }

      this.newQuestionnaireData.deadline = this.TimeView(
        this.newQuestionnaireData.deadline
      );
      this.isSubmit = true;
      await createQuestionnaire(this.newQuestionnaireData)
        .then(res => {
          //console.log(res)
    ...
    pushUp(index) {
      let temp = Object.assign(
        {},
        this.newQuestionnaireData.content[index - 1]
      );
      Object.assign(
        this.newQuestionnaireData.content[index - 1],
        this.newQuestionnaireData.content[index]
      );
    ...
    editContent(data, index) {
      ...
      if (data.type == "single") {
        this.$router.push({
          name: "Cquestionnaire.single",
          params: {
            editDataArr: data.singleContentDataArr,
            editDataIndex: index
          }
        });
      }
      if (data.type == "multi") {
        this.$router.push({
          name: "Cquestionnaire.multi",
          params: {
            editDataArr: data.multiContentDataArr,
            editDataIndex: index
          }
        });
      }
      if (data.type == "qa") {
        this.$router.push({
          name: "Cquestionnaire.questionAnswer",
          params: { editDataArr: data.qaContentDataArr, editDataIndex: index }
        });
      }
      if (data.type == "score") {
        this.$router.push({
          name: "Cquestionnaire.scoring",
          params: { editDataArr: data.scoringDataArr, editDataIndex: index }
        });
      }
    },
    ...
    async saveDraft() {
      ...
      this.isDraft = true;
      await createQuestionnaireTemp(draftData)
        .then(res => {
          console.log(res.data);
          this.isDraft = false;
          this.draftCnt++;
