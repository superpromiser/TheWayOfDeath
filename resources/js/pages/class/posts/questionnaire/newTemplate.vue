<template>
  <v-container class="pa-0" v-if="$isMobile()">
    <v-container class="pa-0 pt-12" v-if="templateNew == true">
      ...
      <v-container class="pa-0">
        ...
        <v-row class="mt-10 ma-0">
          ...
          <v-col
            cols="12"
            v-for="(data, index) in newQuestionnaireTemplateData.content"
            :key="index"
          >
            <!--  single Datas  -->
              ...
              <v-col cols="12">
                <p class="mb-0 d-flex align-center">
                  ...
                  <v-btn
                    ...
                    :disabled="
                      index == newQuestionnaireTemplateData.content.length - 1
                    "
                  >
  ...
  <v-container class="pa-0" v-else>
    <div v-if="templateNew == true">
      <div
        class="px-10 mb-10 z-index-2 banner-custom"
        color="white"
        sticky
        elevation="20"
      >
...
export default {
  ...
  watch: {
    currentPath: {
      handler(val) {
        if (val.name == "classQuestionnaire.templateNew") {
  ...
  methods: {
    ...
    async submit() {
      if (this.newQuestionnaireTemplateData.title.trim() == "") {
        return this.$snackbar.showMessage({
          content: this.lang.share + this.lang.requireTitle,
          color: "error"
        });
      }
      if (this.newQuestionnaireTemplateData.description.trim() == "") {
        return this.$snackbar.showMessage({
          content: this.lang.share + this.lang.requireDescription,
          color: "error"
        });
      }
      if (this.newQuestionnaireTemplateData.content.length == 0) {
        return this.$snackbar.showMessage({
          content: this.lang.share + this.lang.requireContent,
          color: "error"
        });
      }
      this.isSubmit = true;
      await createQuestionnaireTemp(this.newQuestionnaireTemplateData)
        .then(res => {
          this.isSubmit = false;
    ...
    pushUp(index) {
      let temp = Object.assign(
        {},
        this.newQuestionnaireTemplateData.content[index - 1]
      );
      Object.assign(
        this.newQuestionnaireTemplateData.content[index - 1],
        this.newQuestionnaireTemplateData.content[index]
      );
