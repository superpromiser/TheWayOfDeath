      ...
      <v-row class="px-10 ma-0" v-else>
        ...
        >
          <v-card class="mx-auto" max-width="400">
            ...
            <v-card-actions>
              <v-btn text color="#7879ff" @click="selTemp(template)">
                选项
              </v-btn>
              <v-btn text color="#f19861" @click="delTemp(template)">
                删除
              </v-btn>
            </v-card-actions>
          </v-card>
...
export default {
  ...
  methods: {
    ...
    selTemp(tempData) {
      ...
      this.$router.push({
        name: "posts.Cquestionnaire",
        query: { tempData: JSON.stringify(tempData.content) }
      });
    },
    delTemp(tempData) {
      deleteQuestionnaireTemp({ id: tempData.id })
        .then(res => {
          console.log(res.data);
