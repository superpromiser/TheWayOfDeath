<template>
  <v-container class="pa-0" v-if="$isMobile()">
    <v-container class="pa-0 h-100 bg-white mb-16 pb-3 pt-12">
      ...
      <v-row
        v-if="isLoading == true"
        class="ma-0 d-flex justify-center align-center py-16"
      >
        <v-progress-circular
          indeterminate
          color="#7879ff"
...
import { getTemplateList, deleteTemplate } from "~/api/repair";
export default {
  data: () => ({
    lang,
    isSubmit: false,
  ...
  methods: {
    submit() {
      console.log("submit");
      this.$router.push({ name: "Crepair.newTemplate" });
    },
    selTemp(tempData) {
      this.$router.push({
        name: "posts.Crepair",
        query: { tempData: JSON.stringify(tempData.content) }
      });
    },
    async delTemp(tempData) {
      ...
      // return
      this.isDelete = true;
      await deleteTemplate({ id: tempData.id })
        .then(res => {
          console.log(res.data);
          this.isDelete = false;
    ...
    navToNew() {
      this.$router.push({ name: "posts.Crepair" });
    }
  }
};
</script>
