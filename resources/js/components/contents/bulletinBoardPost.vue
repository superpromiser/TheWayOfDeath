  ...
  <v-container v-if="$isMobile()">
    <v-row>
      <v-col cols="12" class="d-flex" @click="showDetail(content)">
        <v-avatar>
          <v-img
            :src="`${baseUrl}/asset/img/appIcon/others/bulletinboard.png`"
            alt="postItem"
          ></v-img>
        </v-avatar>
        <div class="ml-2 d-flex flex-column">
          ...
          >
            {{ lang.bulletin }} ▪ {{ bulletinBoardData.type }}
          </p>
      ...
      <v-col cols="12" class="pb-0">
        <p class="text-wrap mb-0 font-size-0-75">
          <read-more
            more-str="全文"
            :text="bulletinBoardData.content.text"
            link="#"
            less-str="收起"
            :max-chars="250"
          ></read-more>
      ...
      <v-col
        cols="12"
        class="py-0"
        v-if="checkIfAttachExist(bulletinBoardData.content)"
      >
  ...
  <v-container v-else>
    ...
    >
      <v-avatar class="ma-3 school-card-avatar" tile>
        <v-img
          :src="`${baseUrl}/asset/img/newIcon/布告栏.png`"
          alt="postItem"
        ></v-img>
      </v-avatar>
      <div>
