  ...
  <v-col cols="12" class="py-2 pt-0" v-if="$isMobile()">
    <div class="d-flex align-center justify-space-between">
      <p class="mb-0 font-size-0-8">
        浏览 {{ footerInfo.readList ? footerInfo.readList.length : 0 }}次
      </p>
      <v-speed-dial
        v-model="fab"
        direction="left"
      ...
      >
        ...
        <div
          ...
          v-if="user.roleId < 3 || footerInfo.users.id == user.id"
        >
          <v-btn icon color="blue accent-3">
            <v-icon @click="postRemove(footerInfo)"
        ...
        <div class="d-flex align-center ">
          <v-btn icon color="#7879ff" v-if="user.roleId < 3">
            <v-icon
              size="20"
              @click="fixTop(footerInfo)"
              v-if="footerInfo.fixTop == null"
              >mdi-arrow-up
            </v-icon>
            <v-icon size="20" @click="relaseTop(footerInfo.id)" v-else
              >mdi-arrow-down
            </v-icon>
          </v-btn>
        </div>
        ...
        <div class="d-flex align-center ">
          <v-btn icon color="red accent-3" :loading="isLiking">
            <v-icon size="20" v-if="footerInfo.isLiked" @click="removeLike"
              >mdi-heart
            </v-icon>
            <v-icon size="20" v-else @click="addLike"
              >mdi-heart-outline
            </v-icon>
          </v-btn>
          <span>{{
            footerInfo.likes.length > 0 ? footerInfo.likes.length : 0
          }}</span>
        </div>
        <div class="d-flex align-center" @click="showDetail">
          ...
          <span>{{
            footerInfo.comments.length > 0 ? footerInfo.comments.length : 0
          }}</span>
        </div>
      </v-speed-dial>
    </div>
  ...
  <v-col cols="12" class="pt-2 px-10" v-else>
    <div class="d-flex align-center justify-end mb-5">
      ...
      >
        ...
        <p class="mb-0 mr-8">
          {{ footerInfo.readList ? footerInfo.readList.length : 0 }}人
        </p>
      </v-btn>
      <v-btn icon color="red accent-3" :loading="isLiking">
        <v-icon size="25" v-if="footerInfo.isLiked" @click="removeLike"
          >mdi-heart
        </v-icon>
          ...
          <v-card-text>
            <v-row>
              ...
              >
                ...
                <v-avatar v-else class="ma-5" size="50">
                  <v-img :src="user.avatar"></v-img>
                </v-avatar>
                <span>{{ user.name }}</span>
              </v-col>
...
export default {
  ...
  created() {},
  mounted() {
    let index = this.footerInfo.likes
      .map(x => {
        return x.userId;
      })
  ...
  methods: {
    async addLike() {
      this.isLiking = true;
      // this.fab = true;
      await addLike({ postId: this.footerInfo.id })
        .then(res => {
    ...
    async removeLike() {
      this.isLiking = true;
      // this.fab = true;
      await removeLike({ postId: this.footerInfo.id })
        .then(res => {
          let index = this.footerInfo.likes
            .map(x => {
              return x.userId;
    ...
    async showReadUsers() {
      this.isReadCnt = true;
      await getReadCnt({ userList: this.footerInfo.readList })
        .then(res => {
    ...
    postComment() {
      this.isPosting = true;
      addComment({ text: this.commentText, postId: this.footerInfo.id })
        .then(res => {
    ...
    showDetail() {
      ...
      createReadCnt({ postId: this.footerInfo.id })
        .then(res => {
      ...
      if (this.$isMobile()) {
        switch (this.footerInfo.contentId) {
          ...
          case 7:
            if (this.selectedSchoolItem.type == "school") {
              this.$router.push({
                name: "details.schoolRepair",
                params: { schoolId: this.selectedSchoolItem.schoolId },
                query: { autoFocus: true }
              });
            } else {
              this.$router.push({
                name: "details.classRepair",
                params: {
                  schoolId: this.selectedSchoolItem.schoolId,
                  gradeId: this.selectedSchoolItem.gradeId,
                  lessonId: this.selectedSchoolItem.lessonId
            ...
            if (this.selectedSchoolItem.type == "school") {
              ...
              if (this.content.homework.homeworkType == "常规作业") {
                if (this.user.roleId == 3 || this.user.roleId == 7) {
                  this.$router.push({
                    name: "details.homeworkofflineTeacher",
                    params: {
                      schoolId: this.selectedSchoolItem.schoolId,
                      gradeId: this.selectedSchoolItem.gradeId,
                      lessonId: this.selectedSchoolItem.lessonId
                  ...
                  this.$router.push({
                    name: "details.homeworkofflineStudent",
                    params: {
                      schoolId: this.selectedSchoolItem.schoolId,
                      gradeId: this.selectedSchoolItem.gradeId,
                      lessonId: this.selectedSchoolItem.lessonId
                ...
                if (this.user.roleId == 3 || this.user.roleId == 7) {
                  this.$router.push({
                    name: "details.homeworkonlineTeacher",
                    params: {
                      schoolId: this.selectedSchoolItem.schoolId,
                      gradeId: this.selectedSchoolItem.gradeId,
                      lessonId: this.selectedSchoolItem.lessonId
                  ...
                  this.$router.push({
                    name: "details.homeworkonlineStudent",
                    params: {
                      schoolId: this.selectedSchoolItem.schoolId,
                      gradeId: this.selectedSchoolItem.gradeId,
                      lessonId: this.selectedSchoolItem.lessonId
                ...
                if (this.user.roleId == 3 || this.user.roleId == 7) {
                  this.$router.push({
                    name: "details.homeworktestTeacher",
                    params: {
                      schoolId: this.selectedSchoolItem.schoolId,
                      gradeId: this.selectedSchoolItem.gradeId,
                      lessonId: this.selectedSchoolItem.lessonId
          ...
          case 24:
            if (this.selectedSchoolItem.type == "school") {
              ...
              if (this.user.roleId == 5) {
                this.$router.push({
                  name: "details.regnameAnswer",
                  params: { schoolId: this.selectedSchoolItem.schoolId },
                  query: { autoFocus: true }
                });
              } else {
                this.$router.push({
                  name: "details.regnameResult",
                  params: { schoolId: this.selectedSchoolItem.schoolId },
                  query: { autoFocus: true }
                });
              }
          ...
          case 27:
            if (this.selectedSchoolItem.type == "school") {
              this.$router.push({
                name: "classSpace.applications.returnTeam",
                params: { schoolId: this.selectedSchoolItem.schoolId },
                query: { autoFocus: true }
              });
            } else {
