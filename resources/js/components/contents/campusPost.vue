  ...
  <v-container class="pa-0" v-if="$isMobile()">
    <v-row class="ma-0">
      <v-col cols="12" class="d-flex" @click="showDetail(content)">
        <v-avatar>
          <v-img
            :src="`${baseUrl}/asset/img/appIcon/校园文化/校园动态.png`"
            alt="postItem"
          ></v-img>
        </v-avatar>
      ...
      <v-col cols="12" class="py-0 font-size-0-8">
        <v-card tile class="mx-auto" max-width="800">
          <v-img
            height="250"
            :src="`${baseUrl}${content.campus.imgUrl}`"
          ></v-img>
          <v-card-title class="px-3 py-2" style="font-size:0.8rem!important">{{
  ...
  <v-container v-else>
    ...
    >
      <v-avatar class="ma-3 school-card-avatar" tile>
        <v-img
          :src="`${baseUrl}/asset/img/newIcon/校园动态.png`"
          alt="postItem"
        ></v-img>
      </v-avatar>
      <div>
