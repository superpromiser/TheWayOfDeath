<template>
  <fragment>
    <v-card
      tile
      class="mo-glow pa-3 d-flex justify-center align-center"
      v-if="$isMobile()"
      @click="selectItem"
    >
    ...
    <v-card :color="item.color" dark tile v-else>
      <div class="d-flex flex-no-wrap justify-space-between">
        <div>
          <v-card-title class="headline" v-text="item.title"></v-card-title>
          <v-card-subtitle v-text="item.detail"></v-card-subtitle>
          <v-card-actions>
            <v-btn class="ml-2 mt-5" outlined rounded large @click="selectItem">
              {{ btnTxt }}
            </v-btn>
          </v-card-actions>
        </div>
        <v-avatar class="ma-3 post-item-card-avatar" tile>
    ...
    selectItem() {
      ...
      if (this.item.path == "posts.vocation") {
        if (this.user.roleId == 3) {
          ...
          this.$router.push({
            name: "posts.attendance.vacation",
            params: {
              schoolId: this.currentSchoolId,
              gradeId: this.currentGradeId,
              lessonId: this.currentLessonId
