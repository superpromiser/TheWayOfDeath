<template>
  <v-container>
    ...
    <v-row v-if="$isMobile()" v-viewer="moOptions" class="images clearfix">
      <div
        v-for="(img, i) in items.imgUrl"
        :key="i"
        class="image-con text-center"
      >
        <img
          :src="`${baseUrl}${img.path}`"
          :data-source="`${baseUrl}${img.path}`"
          :key="img.path"
          class="mo-image"
        />
      </div>
    ...
    <v-row
      v-else
      v-viewer="$isMobile() ? moOptions : options"
    ...
    <v-row>
      <v-col
        v-for="(video, index) in items.videoUrl"
        :key="index"
        cols="12"
        sm="6"
        md="4"
    ...
    <v-row>
      <v-col
        v-for="(other, index) in items.otherUrl"
        :key="index"
        cols="12"
        sm="6"
        md="4"
  ...
  mounted() {
    for (let i = 0; i < this.items.videoUrl.length; i++) {
      ...
      clonedOption.sources[0].src =
        this.baseUrl + "/uploads/video/" + this.items.videoUrl[i].fileName;
      if (this.$isMobile()) {
